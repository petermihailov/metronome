...
const isGlow = ({
  ...
  return isPlaying ? idx < count - played : idx < count
}
  ...
  useEffect(() => {
    if (svgRef.current) {
      svgRef.current.querySelectorAll('path').forEach((path, idx) => {
        if (isGlow({ isPlaying, count, played, idx })) {
    ...
    <ButtonIcon aria-label="count" className={classes.buttonCounting} {...restProps}>
      <svg ref={svgRef} viewBox="0 0 24 24">
        ...
        <path
          ...
          filter={isGlow({ isPlaying, count, played, idx: 0 }) ? `url(#${filterName})` : undefined}
        />
        <path
          ...
          filter={isGlow({ isPlaying, count, played, idx: 1 }) ? `url(#${filterName})` : undefined}
        />
        <path
          ...
          filter={isGlow({ isPlaying, count, played, idx: 2 }) ? `url(#${filterName})` : undefined}
        />
        <path
          ...
          filter={isGlow({ isPlaying, count, played, idx: 3 }) ? `url(#${filterName})` : undefined}
        />
      </svg>
